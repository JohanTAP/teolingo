...
class SubscriptionService {
  ...
  Future<bool> _activateSubscriptionWithSQLite(String key) async {
    ...
    final int id = await db.insert(_tableName, {
      'key': key,
      'level': level,
    ...
    try {
      ...
      if (kIsWeb && keys.isNotEmpty) {
        ...
        if (maxKeyLevel > storedLevel) {
          debugPrint(
            'Actualizando nivel almacenado de $storedLevel a $maxKeyLevel',
          );
  ...
  Future<String> _getDeviceId() async {
    try {
      ...
      if (deviceId == null) {
        ...
        debugPrint(
          'Nuevo ID de dispositivo generado: ${deviceId.substring(0, 10)}...',
        );
      } else {
        debugPrint(
          'ID de dispositivo existente: ${deviceId.substring(0, 10)}...',
        );
      }
