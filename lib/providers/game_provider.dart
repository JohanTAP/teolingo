...
class GameProvider with ChangeNotifier {
  ...
  Future<void> _checkSubscription() async {
    try {
      if (_isSubscriptionChecked) {
        debugPrint('Suscripción ya verificada, nivel actual: $_maxUnlockedLevel');
        return;
      }
      ...
      debugPrint('Nivel máximo desbloqueado: $_maxUnlockedLevel (anterior: $previousLevel)');
    ...
    try {
      ...
      switch (_currentLevel) {
        case GameLevel.level1:
          ...
          debugPrint('Secuencia nivel 1 (normal): ${_levelSequence.length} letras');
          break;
        case GameLevel.level2:
          // Orden inverso: 21, 20, 19, ..., 0
          _levelSequence = List.generate(totalLetters, (index) => totalLetters - 1 - index);
          debugPrint('Secuencia nivel 2 (inverso): ${_levelSequence.length} letras');
          break;
        case GameLevel.level3:
          // Orden aleatorio
          _levelSequence = List.generate(totalLetters, (index) => index);
          _levelSequence.shuffle();
          debugPrint('Secuencia nivel 3 (aleatorio): ${_levelSequence.length} letras');
          break;
        case GameLevel.level4:
          // Nombre a símbolo: mismo orden que nivel 1 pero diferente presentación
          _levelSequence = List.generate(totalLetters, (index) => index);
          debugPrint('Secuencia nivel 4 (nombre a símbolo): ${_levelSequence.length} letras');
          break;
        case GameLevel.level5:
          // Nombre a símbolo - Orden inverso (de fin a principio)
          _levelSequence = List.generate(totalLetters, (index) => totalLetters - 1 - index);
          debugPrint('Secuencia nivel 5 (nombre a símbolo - orden inverso): ${_levelSequence.length} letras');
          break;
        case GameLevel.level6:
          // Nombre a símbolo - Orden aleatorio
          _levelSequence = List.generate(totalLetters, (index) => index);
          _levelSequence.shuffle();
          debugPrint('Secuencia nivel 6 (nombre a símbolo - orden aleatorio): ${_levelSequence.length} letras');
          break;
        case GameLevel.level7:
          // Símbolo a nombre: mismo orden que nivel 1 pero diferente presentación
          _levelSequence = List.generate(totalLetters, (index) => index);
          debugPrint('Secuencia nivel 7 (símbolo a nombre): ${_levelSequence.length} letras');
          break;
      }
        ...
        if (index == _correctOptionIndex) {
          ...
          do {
            randomIndex = random.nextInt(HebrewLettersData.letters.length);
                   ...
                   _options.any((option) => option.symbol == HebrewLettersData.letters[randomIndex].symbol));
  ...
  Future<void> setLevel(GameLevel level) async {
    try {
      ...
      final bool isUnlocked = await _subscriptionService.isLevelUnlocked(levelNumber);
  ...
  Future<bool> activateSubscription(String key) async {
    try {
      ...
      if (success) {
        debugPrint('Suscripción activada correctamente, actualizando nivel desbloqueado');
        ...
        debugPrint('Nivel máximo desbloqueado después de activar: $_maxUnlockedLevel');
  ...
  bool isLevelUnlocked(GameLevel level) {
    ...
    debugPrint('Verificando si nivel $levelNumber está desbloqueado: $isUnlocked');
    return isUnlocked;
  }
} 